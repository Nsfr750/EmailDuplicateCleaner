    ...
    def view_email_content(self):
        ...
        try:
            ...
            try:
                ...
                # Create notebook for email content
                content_notebook = ttk.Notebook(viewer)
                content_notebook.grid(row=1, column=0, sticky="nsew", padx=10, pady=5)

                # Text view tab
                text_frame = ttk.Frame(content_notebook)
                text_body = scrolledtext.ScrolledText(text_frame, wrap=tk.WORD,
                                                   font=("TkDefaultFont", 10))
                text_body.pack(fill=tk.BOTH, expand=True)
                text_body.insert(tk.END, email_content.get('body', 'No plain text content'))
                text_body.configure(state="disabled")

                # HTML view tab
                html_frame = ttk.Frame(content_notebook)
                html_text = scrolledtext.ScrolledText(html_frame, wrap=tk.WORD)
                html_text.pack(fill=tk.BOTH, expand=True)
                if email_content.get('html'):
                    html_text.insert(tk.END, email_content['html'])
                else:
                    html_text.insert(tk.END, 'No HTML content')
                html_text.configure(state="disabled")

                # Attachments tab
                attach_frame = ttk.Frame(content_notebook)
                attach_tree = ttk.Treeview(attach_frame, columns=('size', 'type'),
                                        show='headings')
                attach_tree.heading('size', text='Size')
                attach_tree.heading('type', text='Type')
                attach_tree.pack(fill=tk.BOTH, expand=True)

                # Add attachments to tree
                for attachment in email_content.get('attachments', []):
                    attach_tree.insert('', 'end', values=(
                        attachment.get('size', 'Unknown'),
                        attachment.get('type', 'Unknown')
                    ))

                # Add tabs to notebook
                content_notebook.add(text_frame, text='Plain Text')
                content_notebook.add(html_frame, text='HTML')
                content_notebook.add(attach_frame, text='Attachments')
                ...
                print(f"Tkinter error: {tcl_err}")
                self.show_error("Interface error - please try again")
            except Exception as e:
                print(f"Error in view_email_content: {str(e)}")
